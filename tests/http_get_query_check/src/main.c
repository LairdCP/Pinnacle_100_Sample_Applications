...
LOG_MODULE_REGISTER(http_get_query_check);
...
#define POST_FILE "log.txt"

#define REQUEST                                                                \
	"GET " HTTP_PATH POST_FILE " HTTP/1.1\r\nHost: " HTTP_HOST             \
	"\r\nConnection: Close\r\n\r\n"
...
static int test_http_get_query_check()
{
...
void main(void)
{
	...
	LOG_INF("Run test_http_get_query_check #1");
	ret = test_http_get_query_check();
	if (ret < 0) {
		LOG_ERR("test_http_get_query_check #1 failed");
		return;
	}
